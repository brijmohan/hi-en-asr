...
for x in test train; do
  steps/make_mfcc.sh --nj $njobs --cmd "$train_cmd" data/$x exp/make_feat/$x $featdir
...
steps/train_mono.sh --nj $trjobs --cmd "$train_cmd" data/train.1k data/lang exp/mono
...
steps/align_si.sh --nj $trjobs --cmd "$train_cmd" \
...
steps/align_si.sh --nj $trjobs --cmd "$train_cmd" --use-graphs true \
   data/train data/lang exp/tri2b exp/tri2b_ali

#  Do MMI on top of LDA+MLLT.
steps/make_denlats.sh --nj $trjobs --cmd "$train_cmd" \
...
steps/decode.sh --config conf/decode.config --iter 4 --nj $tejobs --cmd "$decode_cmd" \
   exp/tri2b/graph data/test exp/tri2b_mmi/decode_it4
steps/decode.sh --config conf/decode.config --iter 3 --nj $tejobs --cmd "$decode_cmd" \
...
(
 ...
 steps/decode_fmllr.sh --config conf/decode.config --nj 3 --cmd "$decode_cmd" \
...
# # run this script.
# steps/cleanup/find_bad_utts.sh --nj 4 --cmd "$train_cmd" data/train data/lang \
...
steps/make_denlats.sh --config conf/decode.config \
   --nj $trjobs --cmd "$train_cmd" --transform-dir exp/tri3b_ali \
...
for iter in 3 4 5 6 7 8; do
 steps/decode_fmmi.sh --nj 3 --config conf/decode.config --cmd "$decode_cmd" --iter $iter \
...
for iter in 3 4 5 6 7 8; do
 steps/decode_fmmi.sh --nj 4 --config conf/decode.config --cmd "$decode_cmd" --iter $iter \
